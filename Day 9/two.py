...
for sequence in data:
    ...
    for index in range(len(delta_sequences) - 1):
        # Prepend to the next sequence in the list the
        # difference between the first value of the current list
        # and the first value of the next list. This difference
        # is the predicted value.
        delta_sequences[index+1].insert(0, int(delta_sequences[index+1][0]) - int(delta_sequences[index][0]))
    # This loop leaves us with the predicted value of the first
    # delta sequence, so we just subtract this value to the
    # first value of the sequence to get our next prediction.
    # This prediction is added to the result for the final
    # answer
    result += int(sequence.split(" ")[0]) - delta_sequences[-1][0]
